...
class ClassTimeTable extends CI_Controller{
	...
	public function __construct(){
		...
		$this->load->model('super_admin/school/ClassTimeTableModel');
	...
	public function data_preview(){
		...
		if($StudentID){
			...
			$data['StudentAddress'] = $this->ClassTimeTableModel->getStudentAddressInfo($StudentID);
			$data['StudentDocuments'] = $this->ClassTimeTableModel->getStudentDocumentInfo($StudentID);
	...
	public function get_table_data(){
		...
		$ClassTimeTableDetails = $this->ClassTimeTableModel->getClassTimeTableDetails($PerPage, $Offset);
		$RowsCount = $this->ClassTimeTableModel->getClassTimeTableDetailsCount();
		$data	   = array("current"=> $Current, "rowCount"=> count($ClassTimeTableDetails), "total"=> $RowsCount,
					"rows"=>$ClassTimeTableDetails
					);
		echo json_encode($data);
	}
	public function data_table(){
		$data['getClassTimeTableDetails'] = $this->ClassTimeTableModel->getClassTimeTableDetails();
		$this->load->view('datatable/school/class-time-table-data-table-view', $data);
	}
	public function create(){
		$this->load->model('super_admin/school/BranchModel');
		$data['BranchList'] = $this->BranchModel->getBranchList();
		$data['WeekList'] = $this->ClassTimeTableModel->getWeeks();
		$data['PeriodList'] = $this->ClassTimeTableModel->getPeriods();
	...
	public function _isClassNameExists($key = NULL, $BranchID = NULL){
		if($key && $BranchID){
			...
			$Result = $this->ClassTimeTableModel->checkClassName($ClassBranchName);
			if($Result > 0){
	...
	public function create_class_time_table(){
		...
		if($this->form_validation->run() !== false){
			...
			$ResultStatus = $this->ClassTimeTableModel->createClassTimeTable($InsertClassTimeTableData);
			if($ResultStatus){
	...
	public function update(){
		...
		if($ClassID){
			$this->load->model('super_admin/school/BranchModel');
			$data['BranchList'] = $this->BranchModel->getBranchList();
			$data['ClassInfo'] = $StudentBranchID = $this->ClassTimeTableModel->getClassInfo($ClassID);
	...
	public function update_class(){
		...
		if($this->form_validation->run() !== false){
			...
			$ResultStatus = $this->ClassTimeTableModel->modifyClass($UpdateClassData, $ClassID);
			if($ResultStatus){
	...
	public function getClassList(){
		...
		if($BranchID){
			$ClassList = $this->ClassTimeTableModel->getClassList($BranchID);
	...
	public function create_employee_subjects_time_table(){
		...
		if($this->form_validation->run() !== false){
			...
			$ResultStatus = $this->ClassTimeTableModel->create_employee_subjects_time_table($InsertAssignSubjectTimeTable);
			if($ResultStatus){
	...
	public function get_event_time_table_info(){
		...
		if($this->form_validation->run() !== false){
			...
			$ResultStatus = $this->ClassTimeTableModel->getEventTimeTableInfo($GetTimeTableInfo);
	...
	public function eventTimeTableContent($ResultStatus){
		$WeekList 	= $this->ClassTimeTableModel->getWeeks();
		$PeriodList = $this->ClassTimeTableModel->getPeriods();
		//head section
