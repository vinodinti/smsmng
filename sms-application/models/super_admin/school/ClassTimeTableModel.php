...
class ClassTimeTableModel extends CI_Model {
	...
	public function getClassTimeTableDetails($PerPage = 0, $Offset = 0){
		if($PerPage)
				$this->db->limit($PerPage, $Offset);
		$this->db->select("titable.*, brc.branch_name, brcls.class_name, brclssec.section_name, CONCAT(emp.employee_first_name,' ', emp.employee_middle_name,' ',emp.employee_last_name) as employee_full_name, sub.subject_name, titable.class_time_table_status as record_status");
		...
		$this->db->where(array("titable.class_time_table_status"=>0,"titable.class_time_table_status"=>1));
		$this->db->order_by("titable.class_time_table_id", "DESC");
		return $this->db->get('')->result();
		
	}
	public function getClassTimeTableDetailsCount(){
				$this->db->select("count(class_time_table_id) as rowcount");
			    $this->db->where(array("class_time_table_status"=>0,"class_time_table_status"=>1));
		return  $this->db->get('school_branch_class_time_table')->row()->rowcount;
	}
